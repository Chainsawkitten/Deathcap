...
public:
	 ...
	 */
	void FinalizeScene(IPLhandle* scene, IPLFinalizeSceneProgressCallback progressCallback);	//Should also create Environment
	 ...
	 */
	SaveData SaveFinalizedScene(IPLhandle* scene); //Should call iplSaveFinalizedScene with null data pointer first to figure out how big to make the data pointer for a second call
	...
	/// Creates a static mesh and adds it to a scene.
	/**
	...
	/// Update function, call ASAP maybe???
	/**
	 *
	 */
	void Update(); // Needs to work in a way that there's always at least one processed audio frame ready to go.
