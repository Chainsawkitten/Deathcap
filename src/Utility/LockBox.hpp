...
namespace Utility {
    ...
    template <typename T>
    LockBox<T>::LockBox(std::shared_ptr<Key> key, T* data) {
        shared = new Shared;
        shared->refs = 1;
        shared->key = key;
        shared->data = data;
    ...
    template <typename T>
    LockBox<T>::LockBox(const LockBox<T>& other) {
        shared = other.shared;
        shared->refs++;
    }

    template <typename T>
    LockBox<T>::LockBox(LockBox<T>&& other)
        : LockBox() {
        swap(*this, other);
    }
    ...
    template <typename T>
    LockBox<T>& LockBox<T>::operator=(LockBox<T> rhs) {
        swap(*this, rhs);
        return *this;
    }
    ...
    template <typename T>
    bool LockBox<T>::Open(std::shared_ptr<Key> key, const std::function<void(T&)>& forValidKey) {
    ...
    template <typename T>
    uint32_t LockBox<T>::RefCount() const {
